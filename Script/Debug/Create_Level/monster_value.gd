...
func _process(delta: float) -> void:
	if co != null and co.MonsterValue.size() != mValue:
		for i in $ScrollContainer/VBoxContainer.get_children():
			i.queue_free()
		mValue = co.MonsterValue.size()
		var k : int = 0
		for i in co.MonsterValue.size():
			var b = preload("res://Scene/Debug/Create_Level/MonsterValue_Button.tscn")
