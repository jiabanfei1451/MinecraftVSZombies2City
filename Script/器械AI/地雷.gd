...
func _ready():
	...
	if 启用 == true:
		...
		var s = create_tween()
		$TNT/TNT3/TNT2.visible = false
		s.tween_property($TNT/TNT3,"position",Vector2(0,59),0.6).set_trans(Tween.TRANS_QUART)
		for i in 6:
			贴图ID = 6 + -i
			await get_tree().create_timer(0.08).timeout
		await s.finished
		$TNT/TNT3/TNT2.visible = true
func _process(delta: float) -> void:
	...
	if 启用 == true:
		...
		if 选定攻击 != null and 开启 == true and 爆了 == false:
			...
			$"检测".visible = false
			visible = false
			var h = preload("res://effect/boom.tscn")
			var boom = h.instantiate()
			boom.scale = Vector2(1.5,1.5)
			boom.position = position
			节点提供变量.特效.add_child(boom)
			var yc = preload("res://Object/一次性音效.tscn")
			var ycs = yc.instantiate()
			ycs.stream = preload("res://Sound/地雷爆炸.ogg")
...
func 激活():
	create_tween().tween_property($TNT/TNT3,"position",Vector2(0,31),1).set_trans(Tween.TRANS_QUINT)
	$"出土".play()
